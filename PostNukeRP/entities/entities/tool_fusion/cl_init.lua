...
function FusGenMenu( um )
	...
	local toMeltdown = um:ReadShort()
	local isNoReturn = um:ReadBool()
	local toCrit = um:ReadShort()
	...
	local gen_frame = vgui.Create("DFrame")
		...
		local genMainLabel = vgui.Create("DLabel", gen_frame)
			...
			genMainLabel:SetText( "Fusion Generator" )
...
usermessage.Hook("fusgen_menu", FusGenMenu)
