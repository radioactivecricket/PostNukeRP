...
SWEP.Purpose		= "Mostly pre-apocalypse Five-seveN (poly-carb\n  lasts a while), it'll keep you alive.  And lucky!"
...
SWEP.IronSightsPos = Vector (4.4955, 0, 3.2553)
SWEP.IronSightsAng = Vector (-0.0127, -0.1746, 0)

function SWEP:Initialize()
	util.PrecacheModel( self.ViewModel )
	util.PrecacheModel( self.WorldModel )
	util.PrecacheSound(self.Primary.Sound)
	util.PrecacheSound("Weapon_FiveSeven.Reload")
...
function SWEP:Reload()
	if self.Weapon:Clip1() < self.Primary.ClipSize then
		...
		self.Weapon:EmitSound("Weapon_FiveSeven.Reload")
