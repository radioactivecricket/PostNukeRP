...
function ENT:Initialize()
	self.Entity:SetModel("models/weapons/w_pist_p228.mdl")
...
function ENT:Use( activator, caller )
	if ( activator:IsPlayer() ) then
		if not activator:HasWeapon("weapon_real_cs_p228") then
			...
			local ent = ents.Create("weapon_real_cs_p228")
			local pos = activator:GetPos() 
			ent:SetModel("models/weapons/w_pist_p228.mdl")
