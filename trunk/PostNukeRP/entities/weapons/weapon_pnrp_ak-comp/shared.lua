...
SWEP.Purpose		= "AK-Composite Rifle is a mid-range \n  powerhouse using SMG rounds."
...
SWEP.IronSightsPos = Vector (6.11, 0, 2.0478)
SWEP.IronSightsAng = Vector (2.683, -0.0918, 0)

function SWEP:Initialize()
	util.PrecacheModel( self.ViewModel )
	util.PrecacheModel( self.WorldModel )
	util.PrecacheSound(self.Primary.Sound)
	util.PrecacheSound("Weapon_AK47.Reload")
...
function SWEP:PrimaryAttack()
	...
	if self.Weapon:GetNWBool("IronSights", false) then
		...
		self:ShootBullet(self.Primary.Damage, self.Primary.NumShots, (self.Primary.Cone - (0.001 * handlingSkill)) / 2, (self.Primary.Recoil - (0.1 * handlingSkill)) / 2 )
	else
		...
		self:ShootBullet(self.Primary.Damage, self.Primary.NumShots, (self.Primary.Cone - (0.001 * handlingSkill)), (self.Primary.Recoil - (0.1 * handlingSkill)) )
	end
...
function SWEP:Reload()
	if self.Weapon:Clip1() < self.Primary.ClipSize then
		...
		self.Weapon:EmitSound("Weapon_AK47.Reload")
