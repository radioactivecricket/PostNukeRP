...
SWEP.Purpose		= "One thing can be said about old designs.\n  They're very easy to do again."
...
SWEP.IronSightsPos = Vector (-5.6554, 0, 2.6828)
...
function SWEP:Initialize()
	...
	util.PrecacheSound("Weapon_357.Reload")
...
function SWEP:PrimaryAttack()
	...
	if self.Weapon:GetDTBool(1) then
		self.Owner:ViewPunch(Angle(math.Rand(-0.5,-2.5) * (self.Primary.Recoil - (0.2 * handlingSkill)), math.Rand(-1,1) * ((self.Primary.Recoil - (0.2 * handlingSkill)) / 2), 0))
		self:ShootBullet(self.Primary.Damage, self.Primary.NumShots, (self.Primary.Cone - (0.0005 * handlingSkill)) / 2, (self.Primary.Recoil - (0.2 * handlingSkill)) / 2 )
	else
		self.Owner:ViewPunch(Angle(math.Rand(-0.5,-2.5) * (self.Primary.Recoil - (0.2 * handlingSkill)), math.Rand(-1,1) * (self.Primary.Recoil - (0.2 * handlingSkill)), 0))
		self:ShootBullet(self.Primary.Damage, self.Primary.NumShots, (self.Primary.Cone - (0.0005 * handlingSkill)), (self.Primary.Recoil - (0.2 * handlingSkill)) )
	end
...
function SWEP:Reload()
	if self.Weapon:Clip1() < self.Primary.ClipSize then
		...
		self.Weapon:EmitSound("Weapon_357.Reload")
