...
SWEP.Author					= "Based on Default GMod Fists"
SWEP.Purpose				= ""
...
function SWEP:DealDamage()
	...
	if ( SERVER && IsValid( tr.Entity ) && ( tr.Entity:IsNPC() || tr.Entity:IsPlayer() || tr.Entity:Health() > 0 ) ) then
		...
		dmginfo:SetDamage( math.random( 3 + skillRating, 6 + math.Round( ((skillRating*20)*0.01)*6 )  ) ) -- 8, 12
		...
		elseif ( anim == "fists_uppercut" ) then
			...
			dmginfo:SetDamage( math.random( 5 + math.Round( ((skillRating*20)*0.01)*7 ), 9 + math.Round( ((skillRating*20)*0.01)*15 ) ) ) -- 12, 24
		end
